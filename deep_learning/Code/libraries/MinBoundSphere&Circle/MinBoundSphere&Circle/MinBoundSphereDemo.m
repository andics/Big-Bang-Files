...
% Compute exact minimum bounding spheres (MBS) using Wezlz's agorithm for 
% the sample meshes and visualize meshes along with their respective MBSs.
% Also compute approximate MBSs usign Ritter's algorith for comparison.
N=numel(Fields);
R=zeros(N,2);
for i=1:N
    ...
    [R(i,1),C,Xb]=ExactMinBoundSphere3D(TR.X);
...
% the former and latter produce approximate and exact solutions, respecitvely
