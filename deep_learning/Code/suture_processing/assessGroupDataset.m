function [data_table_filtered] = assessGroupDataset(net, datasetFolder, num_of_desired_pics, xlRange)
...
if ~isnumeric(num_of_desired_pics)
    num_of_desired_pics = imds_num_elements;
end
increase_value = imds_num_elements / num_of_desired_pics;

textprogressbar('Progress: ');
for i=1:num_of_desired_pics
    progress = round((i/num_of_desired_pics)*100);
    textprogressbar(progress);
    
    current_image_num = round(i * increase_value);
    
   try
     [cross_ratio, cluster_info_1, cluster_info_2] = rateSutureClosure(net, imds, current_image_num, false, false);
       ...
       if isempty([cross_ratio, cluster_info_1, cluster_info_2])
        ...
        cluster_info_2 = ["-", "-", "-", "-"];
        cross_ratio = 0;
        cluster_info_1 = [0, 0, 0, 0, white_stuture_color];
        cluster_info_2 = [0, 0, 0, 0, white_stuture_color];
       end
   ...
   image_variables = {current_image_num, cross_ratio, cluster_info_1(1), cluster_info_1(2), cluster_info_1(3), cluster_info_1(5),...
